...
`<vault>/.oak/pub`, then scaffolds an Astro project (the
...
worktree at `<vault>/.oak/pub`. If the branch is freshly
created, scaffolds the pub-template into the worktree.

Refuses if `<vault>/.oak/pub` already exists — remove it or
use it as-is.
...
### `publish worktree not found at .oak/pub`
...
The directory `<vault>/.oak/pub` is already there. If it's a
valid worktree, just `cd` into it and keep working. If it's stale,
remove it with `git worktree remove --force .oak/pub` and
re-run init.
...
Dev mode reads from `.oak/pub/vault/`, which is only refreshed
